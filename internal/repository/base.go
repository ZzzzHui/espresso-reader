...
import (
	...
	"github.com/ZzzzHui/espresso-reader/internal/repository/schema"
...
func (pg *Database) GetInputs(
	...
	rows, err := pg.db.Query(ctx, query, args)
	if err != nil {
		return nil, fmt.Errorf("advancer repository error (failed querying inputs): %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("advancer repository error (failed reading input rows): %w", err)
	}
